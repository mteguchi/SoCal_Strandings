...
harbor_bay_1 <- st_read("C:\\Users\\tomo.eguchi\\Documents\\Oceans and Maps\\San Diego\\Harbor_Bay", layer = "Harbor_Bay") %>% 
   st_transform(crs = "+proj=longlat +datum=WGS84") 

# I edited SDB in QGIS on my laptop (linux) so that Silver Strand is not included.
# this is for computing coast length
harbor_bay <- st_read("C:\\Users\\tomo.eguchi\\Documents\\Oceans and Maps\\San Diego\\SDB_v2",
               layer = "SDB_v2") %>% 
  st_transform(crs = "+proj=longlat +datum=WGS84") 
...
SS <- st_read("C:\\Users\\tomo.eguchi\\Documents\\Oceans and Maps\\San Diego\\SilverStrand",
               layer = "SilverStrand") %>% 
  st_transform(crs = "+proj=longlat +datum=WGS84") %>%
  st_cast("LINESTRING")

Navy <- st_read("c:\\users\\tomo.eguchi\\Documents\\Oceans and Maps\\Navy_installations_poly", 
