---
title: "Capensis stranding model (v4b only)"
output: html_notebook
---

Using the results in capensis_stranding_model.Rmd, I focus on one model in this document. The model uses Poisson distribution for the number of stranded long-beaked common dolphins along Silver Strand, whereas the negative binomial distribution is used for all beaches except Silver Strand in San Diego County. The additional mortality rate along SS is time dependent ($\mu_{1,t}$), whereas for other beaches it is constant ($\mu_2$).
...
if (!file.exists("RData/DcStranding_out_v4b.rds")){
  ...
  jm.4b <- jags(data = jags.data,
               ...
               DIC = T, 
               parallel=T)
  saveRDS(jm.4b, file = "RData/DcStranding_out_v4b.rds")
  
} else {
  
  jm.4b <- readRDS(file = "RData/DcStranding_out_v4b.rds")
}
