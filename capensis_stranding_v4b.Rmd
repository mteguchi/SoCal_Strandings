...
if (!file.exists(paste0("RData/DcStranding_out_", model.name, ".rds"))){
  ...
  jm <- jags(data = jags.data,
               ...
               DIC = T, 
               parallel=T)
  saveRDS(jm, file = paste0("RData/DcStranding_out_", model.name, ".rds"))
  
} else {
  
  jm <- readRDS(file = paste0("RData/DcStranding_out_", model.name, ".rds"))
}
...
```{r}
mcmc_dens(best.model$samples, c("r", "f", "mu",  "b", "mu1"))
```
