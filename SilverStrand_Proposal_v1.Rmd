...
The number of stranded long-beaked common dolphins per km of beach ranged from `r signif(min(stranding.perKm.all$n_per_km), 3)` to `r signif(max(stranding.perKm.all$n_per_km), 3)` along the coast. When Silver Strand was excluded from the coast line, it ranged from `r signif(min(stranding.perKm.noSS$n_per_km), 3)` to `r signif(max(stranding.perKm.noSS$n_per_km), 3)`. For Silver Strand, however, it ranged from `r signif(min(stranding.perKm.SS$n_per_km), 3)` to `r signif(max(stranding.perKm.SS$n_per_km), 3)`, indicating higher stranding rate at Silver Strand in some years compared with other parts of the coast line (Figure \@ref(fig:plot-StrandingRate)). 
