...
$y_{t,SS} - y_{t,non-SS} = x_t \sim N(\mu, \sigma)$, where 

t = year. If there was no difference between the coast lines, $\mu \approx 0$. Any departure from it provides evidence for higher ($\mu > 0$) or lower ($\mu < 0$) stranding rates at Silver Strand. 
...
mcmc_trace(jm$samples, c("mu", "sigma"))
```

```{r}
mcmc_dens(jm$samples, c("mu", "sigma"))

ggsave(filename = "figures/paired_t_posteriors.png",
       dpi = 600, device = "png")
```
